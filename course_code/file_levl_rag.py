...
from llama import summarize_prompt, classify_prompt, classify_question_topic
...
class ChunkExtractor:
    ...
    def _extract_chunks(self, interaction_id, html_source):
        ...
        return interaction_id, chunks, text

    def extract_file(self, batch_interaction_ids, batch_search_results):
        files, interaction_ids, html_ids = [], [], []
        for idx, search_results in enumerate(batch_search_results):
            interaction_id = batch_interaction_ids[idx]
            for html_id, html_text in enumerate(search_results):
                html_summary = html_text["page_snippet"]
        ...
        return interaction_ids, files, html_ids
    ...
    def extract_chunks(self, batch_interaction_ids, batch_search_results):
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple containing an array of chunks and an array of corresponding interaction IDs.
        """
        ...
        ray_response_refs = [
            ...
            for idx, search_results in enumerate(batch_search_results)
            for idy, html_text in enumerate(search_results)
        ]
        ...
        for response_ref in ray_response_refs:
            interaction_id, _chunks, text= ray.get(response_ref)  # Blocking call until parallel execution is complete
            content = [(_chunk, text) for _chunk in _chunks]
            chunk_dictionary[interaction_id].extend(content)

        # Flatten chunks and keep a map of corresponding interaction_ids
        chunks, texts, chunk_interaction_ids = self._flatten_chunks(chunk_dictionary)

        return chunks, texts, chunk_interaction_ids
    ...
    def _flatten_chunks(self, chunk_dictionary):
        ...
        Parameters:
            chunk_dictionary (defaultdict): 2D Dictionary with interaction IDs, html ID as keys and lists of chunks as values.

        Returns:
            Tuple[np.ndarray, np.ndarray, np.ndarray]: A tuple containing an array of chunks and an array of corresponding interaction IDs and a list of corresponding html IDs.
        """
        chunks = []
        texts = []
        chunk_interaction_ids = []
        for interaction_id, contents in chunk_dictionary.items():
            unique_contents = list(set(contents))
            unique_chunks = [content[0] for content in unique_contents]
            unique_texts = [content[1] for content in unique_contents]
            chunks.extend(unique_chunks)
            texts.extend(unique_texts)
...
class RAGModel:
    ...
    def classify_query(self, queries):
        query_topics = self.summarize_pipe(
            [classify_question_topic(query) for query in queries],
            max_new_tokens=256,
            pad_token_id=self.summarize_pipe.tokenizer.eos_token_id
        )
        return [qtopic[-1]["generated_text"][-1]["content"] for qtopic in query_topics]
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        chunks, texts, chunk_interaction_ids = self.chunk_extractor.extract_chunks(
            batch_interaction_ids, batch_search_results
        )
        unique_texts = list(set(texts))
        unique_topics = self.classify_files(unique_texts)
        unique_embeddings = self.calculate_embeddings(unique_topics)
        topics_embeddings = []
        for text in texts:
            #print("matching")
            for pos, unique_text in enumerate(unique_texts):
                if text == unique_text:
                    topics_embeddings.append(unique_embeddings[pos])
        topics_embeddings = np.array(topics_embeddings)
        assert len(topics_embeddings) == len(chunks)
        ...
        query_topics = self.classify_query(queries)
        query_topics_embeddings = self.calculate_embeddings(query_topics)
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            # Find the file-level embeddings
            relevant_files_embeddings = topics_embeddings[relevant_chunks_mask]
            assert relevant_files_embeddings.shape == relevant_chunks_embeddings.shape
            #Calculate cosine similarity between query and file-level embeddings
            file_cosine_scores = (relevant_files_embeddings * query_topics_embeddings).sum(1)

            # Calculate cosine similarity between query and chunk embeddings, multiplied by file-level scores.
            #print(relevant_files_embeddings.shape)
            #print(file_cosine_scores.shape)
            #print(relevant_files_embeddings.shape)
            #print(query_embedding.shape)
            cosine_scores = (relevant_chunks_embeddings * query_embedding.T).sum(1) * file_cosine_scores
