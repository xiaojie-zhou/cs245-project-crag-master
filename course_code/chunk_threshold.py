...
# Chunk Similarity Threshold Parameter
CHUNK_SIMILARITY_THRESHOLD_MULTIPLIER = float(os.getenv("THRESHOLD", 0.3))
print("CHUNK_SIMILARITY_THRESHOLD_MULTIPLIER", CHUNK_SIMILARITY_THRESHOLD_MULTIPLIER)
...
class RAGModel:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        for _idx, interaction_id in enumerate(batch_interaction_ids):
            ...
            # ---MINIMUM THRESHOLD APPROACH STARTS---
            # Calculate the similarity threshold
            total_similarity = cosine_scores.sum()
            threshold = CHUNK_SIMILARITY_THRESHOLD_MULTIPLIER * total_similarity

            # Sort chunks by similarity in descending order
            sorted_indices = (-cosine_scores).argsort()
            sorted_chunks = relevant_chunks[sorted_indices]
            sorted_scores = cosine_scores[sorted_indices]

            # Select sentences until the cumulative similarity exceeds the threshold
            cumulative_similarity = 0.0
            selected_chunks = []
            for score, chunk in zip(sorted_scores, sorted_chunks):
                cumulative_similarity += score
                selected_chunks.append(chunk)
                if cumulative_similarity >= threshold:
                    break
            
            batch_retrieval_results.append(selected_chunks)

            # ---MINIMUM THRESHOLD APPROACH ENDS---
